...
def simple(safeness:float)->bool:
    ...
    if player.is_out or (player.hand.get_value() < croup.hand.get_value() and not croup.is_out):
        lost = True
        logging.info("Player lost because he has %s and croupier has %s",\
                     player.hand.get_value(),croup.hand.get_value())
    else:
        logging.info("Player won because he has %s and croupier has %s",\
                     player.hand.get_value(),croup.hand.get_value())
    return lost
...
def wrapper():
    ...
    """
    n = 10 or int(sys.argv[1]) #pylint: disable=invalid-name
    step = 0.1
    safeness = 0.5
    while safeness < 1:
        for _ in range(n):
