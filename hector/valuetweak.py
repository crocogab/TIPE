...
def simple(safeness:float)->bool:
    ...
    mytree = tree.create_game_tree(tree.MAINTREE, 0)
    ...
    while ((not player.is_out or not player.stopped) and (not croup.is_out or not croup.stopped)):
        if not player.is_out and not player.stopped:
            ...
            if (automate(cards_string,safeness,mytree)[0] or player.hand.get_value() == 0):
                logging.info("HIT! %s cards with a value of %s",len(player.hand.l_cards),player.hand.get_value())
                my_game.give_card(player)
                player.check()
            else:
                player.stopped = True
                logging.info("STOP! %s cards with a value of %s. Surival rate: %s",len(player.hand.l_cards),player.hand.get_value(),automate(cards_string,safeness,mytree)[1])
    ...
    if player.is_out or (player.hand.get_value() < croup.hand.get_value() and not croup.is_out):
        lost = True
        logging.info("Player lost because he has %s and croupier has %s",player.hand.get_value(),croup.hand.get_value())
    else: 
        logging.info("Player won because he has %s and croupier has %s",player.hand.get_value(),croup.hand.get_value())
    return lost
...
def automate(card_string:str,safeness:float,mytree:tree.Tree) -> tuple:
    ...
    mytree.survival_meth() #TODO  pas besoin de calculer la survie pour tout l'arbre, seulement pour les noeuds qui nous intéressent 
    for card in card_string:
        mytree = mytree.navigate(card)
    return mytree.shouldtake(safeness),mytree.survival

def wrapper():
    n = 10 or int(sys.argv[1])
    step = 0.1
    safeness = 0.5
    while safeness < 1:
        for _ in range(n):
